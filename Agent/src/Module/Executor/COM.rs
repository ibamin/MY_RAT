...
pub unsafe fn execute_command(
    ...
    // BSTR을 PCWSTR로 변환
    let method_name_wide: Vec<u16> = method_name
        .to_string()
        .encode_utf16()
    ...
    shell.GetIDsOfNames(&GUID::zeroed(), &method_pcwstr, 1, 0, &mut dispid)?;
            ...
            match status_result {
                Ok(status) => {
                    if status.Anonymous.Anonymous.vt == VT_I4 {
                        if status.Anonymous.Anonymous.Anonymous.lVal == 1 {
                            // WshFinished
                            break;
                        }
                    }
...
unsafe fn get_property_simple(
    ...
    // String을 PCWSTR로 변환
    let prop_wide: Vec<u16> = prop_bstr
        .to_string()
        .encode_utf16()
        .chain(std::iter::once(0))
        .collect();
    ...
    obj.GetIDsOfNames(&GUID::zeroed(), &prop_pcwstr, 1, 0, &mut dispid)?;
