...
pub fn Syscall_PowerShell_Execute(Command: String) -> Result<String> {
    unsafe {
        ...
        let command = format!(
            "powershell.exe -Command \"{} | Out-File -FilePath '{}' -Encoding UTF8\"",
            Command,
        ...
        match result {
            Ok(_) => {
                ...
                if wait_result == WAIT_OBJECT_0 {
                    ...
                    output = format!(
                        "[TIMEOUT] 프로세스가 30초 내에 완료되지 않았습니다. WaitResult: {:?}",
                        wait_result
                    );
                }
...
pub fn create_hidden_process() -> Result<()> {
    Syscall_CreateProcessW(
        "C:\\Windows\\System32\\cmd.exe",
        Some("cmd.exe /c dir C:\\"),
        true,
    )
}
